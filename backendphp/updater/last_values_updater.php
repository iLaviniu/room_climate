<?php
    ...
    $sql_get_rows = "select * from `".$today_table."` order by date_time desc LIMIT 1";
    $sql_get_rows_result = (MySQLConnection::getConnection())->query($sql_get_rows);

    if ($row = $sql_get_rows_result->fetch_assoc()) {
    ...
    $sql_last_status = "select * from `arduino` where status='LAST_SENT' LIMIT 1";
    $sql_last_status_result = (MySQLConnection::getConnection())->query($sql_last_status);
    if ($row_last_sent = $sql_last_status_result->fetch_assoc()) {
    ...
    if($minutes_since_last_status_Int > 1) { /* checking arduino status each minute. */
