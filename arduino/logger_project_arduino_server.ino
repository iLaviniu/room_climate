...
#include "EthernetServiceNodeJs.h"
...
void wifiConnectionTest() {
  ...
  for(trial = 0; ( (trial < 1) && (wifiStatus == false) ); trial++) {
    Serial.print("loop: wifi connection test trial ");
...
void sendStatusToServerPHP() {
  ...
  for(tryIndex = 0; ((tryIndex < 5) && (sendRequested == true)); tryIndex++) {
    ...
    sendStatusHttpRequestPHP(&client, httpResponse);
...
void sendDataToServerPHP() {
  ...
  for(tryIndex = 0; ((tryIndex < 5) && (sendRequested == true)); tryIndex++) {
    httpResponse = (char*)malloc(35 * sizeof(char));
    httpResponseStart = httpResponse;
    sendDataHttpRequestPHP(&client, httpResponse, dht11SensorValues, bmp280SensorValues, periodMinutesDataSending);
...
/*NodeJs Server*/
void sendStatusToServerNodeJs() {
  char * httpResponse;
  char * httpResponseStart;
  char buffer[130] = "no_response_from_server\0";
  byte tryIndex;
  bool sendRequested = true;

  for(tryIndex = 0; ((tryIndex < 5) && (sendRequested == true)); tryIndex++) {
    httpResponse = (char*)malloc(130 * sizeof(char));
    httpResponseStart = httpResponse;
    sendStatusHttpRequestNodeJs(&client, httpResponse);
...
void sendDataToServerNodeJs() {
  ...
  for(tryIndex = 0; ((tryIndex < 5) && (sendRequested == true)); tryIndex++) {
    httpResponse = (char*)malloc(130 * sizeof(char));
    httpResponseStart = httpResponse;
    sendDataHttpRequestNodeJs(&client, httpResponse, dht11SensorValues, bmp280SensorValues);
...
void setup() {
  ...
  else
  {
    ...
    if(installedFirmwareVersion < WIFI_FIRMWARE_LATEST_VERSION)
    {
      ...
      Serial.print("setup: latest firmware version ");
      Serial.println(WIFI_FIRMWARE_LATEST_VERSION);
      Serial.println("setup: please upgrade the firmware");
    }
    else
    {
      Serial.print("setup: installed firmware version ");
      Serial.println(WIFI_FIRMWARE_LATEST_VERSION);
    }
  ...
  if( (isSetupDone == true) && (isWifiActive == true) )
  {
    ...
    if(millis() - timeNowStatusSending > periodMillisStatusSending) {
      ...
      if(WiFi.status() == WL_CONNECTED) {
        ...
        Serial.println("loop: PHP Application");
        Serial.println("loop: sending status to server...");
        sendStatusToServerPHP();
        Serial.flush();
        Serial.println();
        Serial.println();
        /*****************************/
        ...
        /*NodeJs Application*/
        // Serial.println("loop: 1 minute passed");
        // Serial.println("loop: NodeJs Application");
        // Serial.println("loop: sending status to server...");
        // sendStatusToServerNodeJs();
        // Serial.flush();
        // Serial.println();
        // Serial.println();
        /*****************************/
